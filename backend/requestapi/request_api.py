...
import json
import pika
import logging # Keep basic logging config
...
logging.basicConfig(level=logging.WARNING, format='%(asctime)s - %(levelname)s - %(message)s') # Log only warnings and above
...
def trigger_inventory_update(request_data):
    ...
    try:
        ...
        logger.error(f"Inventory Update: Unexpected error: {e}") # Keep critical errors
        return False

@app.route('/health', methods=['GET'])
def health_check():
    """Basic health check."""
    # Simplified: Just check if the endpoint returns anything without a 5xx error
...
def create_request():
        ...
        for recipient_id_str, items in data.items():
             ...
             for item_data in items:
                try:
                    ...
                    # Call Notification Service to create record
                    post_url = f"{NOTIFICATION_API_URL}/{sender_id}"
                    response = requests.post(post_url, json=notification_payload, timeout=10)

                    if not response.ok:
                        has_errors = True
                        continue # Skip to next item on failure

                    response_data = response.json()
                    if response_data.get("code") != 200:
                        has_errors = True
                        continue # Skip on business error

                    # Extract ID for publishing (if needed, otherwise ignore)
                    record_id = response_data.get("data", {}).get("response", [{}])[0].get('id')
                    if not record_id:
                         has_errors = True
                         continue # Skip if ID missing

                    results.append({"id": record_id, "recipient_id": recipient_id_str}) # Track success

                    # Publish message
                    routing_key = f"charity.{recipient_id_str}"
                    message_payload = {
                        'event': 'new_request', 'request_id': record_id, 'sender_id': sender_id,
                        'recipient_id': int(recipient_id_str), 'item_id': item_data['item_id'], 'quantity': item_data['quantity'],
                        'notification': item_data['notification'], 'category': item_data['category'],
                        'timestamp': datetime.now().isoformat()
                    }
                    publish_message(routing_key, message_payload) # Fire and forget publishing error

                except Exception: # Catch broad errors during item processing
                     has_errors = True
    ...
    except Exception as e: # Catch errors before looping
        logger.error(f"Error processing create_request: {e}") # Keep critical errors
        return jsonify({"code": 500, "status": "error", "message": "Server error"}), 500
...
def get_requests():
    ...
    try:
        # Determine Notification API URL
        api_url = f"{NOTIFICATION_API_URL}/new/{charity_id}" if new_only else f"{NOTIFICATION_API_URL}/{charity_id}"

        response = requests.get(api_url, timeout=10)

        if not response.ok:
             # Handle 404 from notification service as "no results" -> return 200 with empty list
            if response.status_code == 404:
                try:
                    # Check if the body indicates a specific 404 code as well
                    error_data = response.json()
                    if error_data.get("code") == 404:
                       return jsonify({"code": 200, "data": []}), 200
                except Exception: pass # Ignore if body isn't JSON or doesn't have code

            # For other errors, return a generic error
            return jsonify({"code": 502, "error": "Error fetching requests from notification service"}), 502 # Bad Gateway

        response_data = response.json()
        # Handle 404 code within a 200 response as well
        if response_data.get("code") == 404:
            return jsonify({"code": 200, "data": []}), 200
        if response_data.get("code") != 200:
        ...
        # Apply filtering locally (simplified)
        ...
        logger.error(f"Error in get_requests: {e}") # Keep critical errors
...
def update_request_status(request_id_str):
    ...
    try:
        # 1. Fetch original request data (inefficiently, requires GET /notification/{responder_id})
        #    This is needed to get details for the PUT payload and check permissions.
        get_url = f"{NOTIFICATION_API_URL}/{responder_id_int}"
        get_response = requests.get(get_url, timeout=10)

        if not get_response.ok:
             return jsonify({"code": 502, "error": "Could not retrieve request details"}), 502

        get_data = get_response.json()
        if get_data.get("code") != 200 and get_data.get("code") != 404: # Allow 404 here
        ...
        if not request_data:
            return jsonify({"code": 404, "error": "Request not found or responder not involved"}), 404

        # 3. Verify permissions
        if responder_id_int != request_data.get('recipient_id'):
             return jsonify({"code": 403, "error": "Forbidden"}), 403

        # --- Use PUT /notification for all status updates ---
        # 4. Prepare payload and call PUT /notification
        update_payload = {
            'id': request_id_str,
            'Recipient': request_data.get('recipient_id'),
            'Notification': request_data.get('notification'), # Include original details
            'Quantity': request_data.get('quantity'),
            'Status': new_status # The new status
        }
        update_response = requests.put(NOTIFICATION_API_URL, json=update_payload, timeout=10)

        # 5. Check update response
        if not update_response.ok:
             return jsonify({"code": 502, "error": "Failed to update status via notification service"}), 502

        update_data = update_response.json()
        if update_data.get("code") != 200:
             return jsonify({"code": 502, "error": "Notification service reported error on update"}), 502

        # Get updated data (simplified - assumes success if code is 200)
        updated_record_data = update_data.get("data", {}).get("response", [{}])[0]
        # Fallback if response structure is odd but code was 200
        if not updated_record_data:
            updated_record_data = {**request_data, "status": new_status}

        # 6. Publish & Trigger Inventory for 'accepted'/'rejected' (simplified error handling)
        if new_status in ['accepted', 'rejected']:
            sender_id = request_data.get('sender_id')
            if sender_id:
                routing_key = f"charity.{sender_id}"
                message_payload = {
                    'event': 'request_updated', 'request_id': request_id_str, 'responder_id': responder_id_int,
                    'original_sender_id': sender_id, 'status': new_status, 'item_id': request_data.get('item_id'),
                    # Include other relevant fields from request_data if needed by listener
                    'timestamp': datetime.now().isoformat()
                }
                publish_message(routing_key, message_payload) # Fire and forget publish
                ...
                if new_status == 'accepted':
                     ...
                     trigger_inventory_update(inventory_details) # Fire and forget trigger

        return jsonify({"code": 200, "data": updated_record_data}), 200

    except Exception as e:
        logger.error(f"Error updating status for {request_id_str}: {e}") # Keep critical errors
    ...
    try:
        ...
        try:
            ...
            if container.status != 'running':
                container.start()
            return jsonify({"code": 200, "status": "success", "message": "Listener running"}), 200
        except docker.errors.NotFound:
            pass # Create new one

        listener_env = {
            "CHARITY_ID": str(charity_id), "RABBITMQ_HOST": RABBITMQ_HOST,
            "RABBITMQ_PORT": str(RABBITMQ_PORT), "RABBITMQ_USER": RABBITMQ_USER,
            "RABBITMQ_PASS": RABBITMQ_PASS, "EXCHANGE_NAME": EXCHANGE_NAME,
            "API_BASE_URL": f"http://request-api:5101", # Assumed internal URL/port
        ...
        logger.error(f"Listener start error for {charity_id}: {e}") # Keep critical error
...
def stop_listener(charity_id):
    ...
    try:
        ...
        logger.error(f"Listener stop error for {charity_id}: {e}") # Keep critical error
        return jsonify({"code": 500, "status": "error", "message": "Failed to stop listener"}), 500

# Main execution block
if __name__ == '__main__':
    port = int(os.environ.get('FLASK_RUN_PORT', 5101))
    host = os.environ.get('FLASK_RUN_HOST', '0.0.0.0')
    # Keep debug off by default unless explicitly set
